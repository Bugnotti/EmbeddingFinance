...
function Setting({ icon: Icon, label, value, onPress, trailing }: { icon: typeof Bell; label: string; value: string; onPress?: () => void; trailing?: ReactNode }) {
  const content = <><View style={styles.settingIcon}><Icon color={colors.primary} size={18} /></View><AppText variant="small" style={{ flex: 1, fontWeight: '700' }}>{label}</AppText><AppText variant="caption" color={colors.inkMuted}>{value}</AppText>{trailing ?? <ChevronRight color={colors.inkSoft} size={17} />}</>;
